      ...
      <div className="bg-brand-white flex h-full w-175 flex-col justify-between border-l-[1px] border-gray-300 px-11 py-8 shadow-[-4px_0_10px_rgba(0,0,0,0.1)]">
        <div className="flex flex-col">
          ...
          <div className="flex flex-col gap-4">
            {POLICY_DETAIL.isActive ? <div>활성화</div> : <div>비활성화</div>}
            <span>{POLICY_DETAIL.name}</span>
            <span>{POLICY_DETAIL.description}</span>
          </div>
          ...
          <span>정책 활성화 시 즉시 모든 유저에게 적용됩니다</span>
          <div>
            <div>
              <input type="radio" />
              <span>정책 수정 이후에만 적용하기</span>
            </div>
            <div>
              <input type="radio" />
              <span>기존 값 덮어쓰기</span>
            </div>
            <span>유저가 설정했던 모든 값이 덮어씌워집니다.</span>
          </div>
        </div>
